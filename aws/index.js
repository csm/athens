...
async function login(event, context) {
    ...
    if (userInfo.expires != null && userInfo.expires < Date.now()) {
        return {
            statusCode: 401,
            headers: {'Content-type': 'application/json'},
            body: JSON.stringify({error: 'Account expired'})
        };
    }
    if (privateKey == null) {
        if (ssm == null) {
...
function _getSignedUrl(s3client, op, args) {
...
async function getSignedUrl(event, context) {
    ...
    if (verb == null || (verb != 'get' && verb != 'put')) {
        return {
            statusCode: 400,
            headers: {'Content-type': 'application/json'},
            body: JSON.stringify({error: 'invalid verb (should be get or put)'})
        }
    }
    if (s3 == null) {
        s3 = new AWS.S3();
    }
    let url = await _getSignedUrl(s3, verb == 'get' ? 'getObject' : 'putObject', {
        Bucket: process.env.S3_BUCKET,
        Key: `athens/${uuid}/${path}`
    });
    return {
        statusCode: 200,
        headers: {"content-type": "application/json"},
